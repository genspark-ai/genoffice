...
export interface Analytics {
  /** true when keys are present (events may still be gated by the opt-out) */
  readonly active: boolean
  /** fire-and-forget event; never throws, no-op without keys or when opted out */
...
/** absent or malformed means enabled — opting out writes an explicit false */
...
export function createAnalytics(options: AnalyticsOptions): Analytics {
  const { keys, clientId, isEnabled, baseParams = () => ({}), fetchFn = fetch } = options
