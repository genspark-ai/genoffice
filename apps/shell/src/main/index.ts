...
function resolveAnalyticsKeys(): AnalyticsKeys | null {
  // env override for local/dev verification, then the credentials injected
  // into the packaged package.json by electron-builder extraMetadata
  const fromEnv = extractAnalyticsKeys({
    genofficeAnalytics: {
      measurementId: process.env.GENOFFICE_GA4_MEASUREMENT_ID,
      apiSecret: process.env.GENOFFICE_GA4_API_SECRET,
    },
  })
  if (fromEnv) return fromEnv
  try {
    return extractAnalyticsKeys(
...
function initAnalytics(): void {
  try {
    analytics = createAnalytics({
      ...
      clientId: ensureAnalyticsClientId(APP_SETTINGS_PATH()),
...
function registerHomeIpc(): void {
  ...
  ipcMain.handle(HOME_CHANNELS.setAnalyticsEnabled, (_event, enabled: unknown) => {
    const next = enabled !== false
    cachedAnalyticsEnabled = next
    writeAppSetting(APP_SETTINGS_PATH(), ANALYTICS_ENABLED_KEY, next)
  })

  // fire-and-forget usage events from renderers (home + editor tabs); the
  // name and params are re-validated main-side, everything else is dropped
  ipcMain.on(HOME_CHANNELS.analyticsTrack, (_event, name: unknown, params: unknown) => {
