...
describe('analyticsEnabledFrom', () => {
  it('defaults to enabled when the key is absent or malformed', () => {
...
describe('createAnalytics', () => {
  ...
  it('does not send while the user is opted out, and resumes when re-enabled', () => {
    const fetchFn = okFetch()
    let enabled = false
    const analytics = createAnalytics({
      keys: KEYS,
  ...
  it('drops events with invalid names instead of sending garbage', () => {
    const fetchFn = okFetch()
    const analytics = createAnalytics({ keys: KEYS, clientId: 'c', isEnabled: () => true, fetchFn })
