...
export function spliceParagraphText(
  ...
  if (end <= paragraph.length) return `${paragraph.slice(0, start)}${insert}${paragraph.slice(end)}`
  const chars = Array.from(paragraph)
  if (end > chars.length) throw new Error(SELECTION_NOT_IN_PARAGRAPH)
  return `${chars.slice(0, start).join('')}${insert}${chars.slice(end).join('')}`
}
