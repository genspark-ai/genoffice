...
export function App() {
  ...
  const measureSingleFlow = useCallback(
    function run<T>(pm: HTMLElement, fn: () => T): T {
      if (colMode === 'none' || viewMode !== 'print') return fn()
  ...
  useEffect(() => {
    ...
    const remeasure = () => {
      ...
      const measured = measureSingleFlow(pm, () => {
        ...
        const secList = sections.length > 0 ? liveSections(sections, blocks, sectBreaks) : null
      ...
      if (editor) {
        ...
        const colSpecs =
          viewMode === 'print' && !readMode && colMode === 'mixed' && secList
            ...
            : []
        setColumnLayout(editor.view, colSpecs)
