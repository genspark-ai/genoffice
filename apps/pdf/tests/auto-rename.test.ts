...
import type { PdfAutoRenameResult } from '../src/shared/ipc'
import { PDF_CHANNELS } from '../src/shared/ipc'
import { createPdfView, markPdfUntitledPath, setPdfRenamedHook } from '../src/main/pdf-main'
...
describe('pdf auto-rename', () => {
  ...
  it('dedupes when the target name already exists', () => {
    const path = makePdfFile()
    writeFileSync(join(path, '..', 'Report.pdf'), '%PDF-1.4\n%%EOF\n')
    markPdfUntitledPath(path)
    createPdfView(path)
