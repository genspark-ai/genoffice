...
describe('buildPositionMatrix', () => {
  it('aggregates the same offsets across areas', () => {
    const matrix = buildPositionMatrix('sum', [area('Q1', 1, 1, 2, 2), area('Q2', 0, 4, 2, 2)])
...
describe('buildLabelMatrix', () => {
  ...
  it('rejects max/min, single-column areas, and label-less sources', () => {
    expect(buildLabelMatrix('max', areas, labels, { row: 0, column: 0 })).toMatch(/不支持/)
    expect(
      buildLabelMatrix('sum', [area('Q1', 0, 0, 2, 1)], [['A']], { row: 0, column: 0 }),
    ).toMatch(/两列/)
    expect(buildLabelMatrix('sum', areas, [[''], []], { row: 0, column: 0 })).toMatch(
      /没有找到标签/,
    )
  })
})
...
describe('targetOverlapsSource', () => {
  ...
  it('detects same-sheet overlap and clears disjoint or cross-sheet targets', () => {
    expect(
      targetOverlapsSource(sources, 'Sheet1', { row: 2, column: 1, rows: 2, columns: 2 }),
    ).toBe(true)
    expect(
      targetOverlapsSource(sources, 'Sheet1', { row: 0, column: 2, rows: 3, columns: 2 }),
    ).toBe(false)
    expect(
      targetOverlapsSource(sources, 'Sheet2', { row: 0, column: 0, rows: 3, columns: 2 }),
    ).toBe(false)
  })
})
