...
async function openAiCompatibleTurn(
  ...
  const runTurn = async (): Promise<{
    ...
    for await (const line of sseLines(response.body, onBytes)) {
      ...
      // DeepSeek streams thinking before the answer; capture it even when it is
      // an empty string (the API still requires it back on tool-call turns).
  ...
  const attempts = retryEmptyTurns ? 2 : 1
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    if (retryEmptyTurns && attempt < attempts - 1) continue
    if (stopReason) cb.onStopReason?.(stopReason)
    if (retryEmptyTurns) throw new Error('The model returned no content (empty stream)')
    return
  }
}
